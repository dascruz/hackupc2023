...
namespace Reqs {
    ...
    public class Requests
    {
        ...
        public IEnumerator CustomPostReq(string uri, string message, System.Action<string> callback)
        {
            uri += message;
            UnityWebRequest webRequest = new UnityWebRequest(uri, "GET");
            ...
            switch (webRequest.result)
            {
                ...
                case UnityWebRequest.Result.ProtocolError:
                    ...
                    //if (uri.EndsWith("/start")) {
                        ...
                        {
                            Debug.Log("retrying...");
                            yield return CustomPostReq(uri, message, callback);
                        }
                        if (webRequest.error == "HTTP/1.1 504 Gateway Timeout")
                        {
                            Debug.Log("retrying...");
                            yield return CustomPostReq(uri, message, callback);
                        }
                    ...
                    break;
                case UnityWebRequest.Result.Success:
                    var text = webRequest.downloadHandler.text.Replace(": ", "");
                    text = text.Replace("- ", "");
                    callback(text);
                    break;
            }
